...
fn system_prompt(
    ...
    format!(
        "Ты подбираешь варианты ответа в переписке от лица пользователя — обычного живого человека, а не ассистента.\n\
         ...
         - В диалоге могут быть и прошлые ответы самого пользователя. НИКОГДА не повторяй мысли, шутки и формулировки, которые в диалоге уже звучали (чьи бы они ни были) — каждый ответ должен добавлять что-то НОВОЕ и двигать разговор дальше, а не пережёвывать сказанное.\n\
         - Три варианта должны заметно отличаться друг от друга по смыслу и заходу — это три разных способа ответить, а не три пересказа одной мысли.\n\
...
pub async fn generate(
    ...
    let body = json!({
        ...
        "top_p": 0.95,
        "max_tokens": 700,
        // штрафы за повторение уже сказанного (и в диалоге, и в самих вариантах)
